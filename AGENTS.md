...
7. 推送 GitHub 前必须检查 `gh` 登录态账号，确认 `gh auth status` 或 `gh api user --jq .login` 显示的账号为 `Yozakura9364`；不得使用其他 GitHub 账号推送本项目。
8. 当我已经同意当前改动，并要求“继续”“继续推进”“继续向后推”或进入下一阶段时，必须先问我是否需要把当前改动提交 commit / 推送 GitHub；除非我已经明确说不要提交或只继续本地开发。
