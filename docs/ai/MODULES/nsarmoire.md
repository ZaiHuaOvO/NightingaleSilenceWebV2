...
- 2026-07-09 衣柜分析行动项过滤扩展；2026-07-11 已收紧：雇员市场上架物品和雇员已装备物品只保留在原始 snapshot、缓存和调试数据中，不进入任何功能分析。雇员市场判定条件为 `inventoryType=12002` 或雇员容器名以 `市场` 结尾；雇员已装备判定条件为 `inventoryType=11000` 或雇员容器名以 `已装备` 结尾。基础统计、容器分布、商城拥有状态、收藏柜、套装、可交易、生产采集复制品、染色风险、重复物品和同模型物品都必须使用过滤后的 actionable snapshot。
- 2026-07-11 snapshot 支持普通装备 `glamourId`。如果物品 `glamourId > 0`，说明该条目被投影覆盖；网页和插件分析时不把该条目的 `dyes` 当作装备本体染色，也不参与贵重染剂、灰化沉底或清染色风险。投影台真实保存物品的染色仍按投影台数据处理；投影台套装容器继续使用既有 `dyes` bitmask 表示缺件状态，不套用普通染色归零。
- 2026-07-11 套装幻影化桶覆盖的散件需要全局识别：套装桶已覆盖的散件作为逻辑拥有来源参与套装、商城和相关收集状态；角色实际持有的同 itemId 散件仍保留在清理建议、重复物品、同模型物品、生产采集复制品和染色风险等实体条目分析中，不能被桶覆盖状态过滤掉。
- 2026-07-11 套装散件状态改为统一解析：每个散件同时记录 `storedByBucket`、普通持有条目和最终状态。套装桶覆盖优先显示为“已收纳”，即使同一 `itemId` 仍在背包、兵装库、鞍囊或雇员中，普通实体条目也必须保留给清理、重复、同模型、复制品和染色风险分析；只有未被套装桶覆盖的普通条目才进入“散件待收纳”。网页端的套装、商城和清理关联提示共用这套结果，不再由页面单独猜测位置。
...
5. `hq`、`quantity`、`dyes`、`glamourId`、`spiritbond` 属于用户拥有的物品实例状态，不来自静态 CSV。`dyes` 记录该条目当前可读染色，`glamourId > 0` 表示普通装备被投影覆盖；分析时被投影覆盖的普通装备不把 `dyes` 当作本体染色。后续 helper / 插件抓数据阶段再按真实可读字段扩展耐久、魔晶石、制造者签名等状态。
