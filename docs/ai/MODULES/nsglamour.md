...
`NSGlamourTemplateWorkspace.vue` 只保留模板设置、装备行展示数据、图片槽 UI 和页面级生命周期编排。装备搜索/染剂交互、图片交互和图片持久化不再直接写入主工作台：
...
- `useGlamourTemplateEquipmentEditor.ts`：装备搜索防抖/竞态、染剂按语言缓存和编辑事件编排。
