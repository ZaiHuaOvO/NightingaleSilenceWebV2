...
`NSGlamourTemplateWorkspace.vue` 只保留模板设置、装备/染剂编辑、图片槽和页面级生命周期编排。弹层与 Canvas 调度按以下边界拆分：

- `NSGlamourTemplateSelectorDialog.vue`：模板筛选、模板卡片和打开时焦点。
- `NSGlamourTemplateImportDialog.vue`：链接输入、状态和打开时焦点；链接归一与提交仍由工作台编排。
- `NSGlamourTemplateCropDialog.vue`：裁剪 Canvas、缩放、拖拽、滚轮和裁剪数据输出；图片持久化与队列仍由工作台编排。
