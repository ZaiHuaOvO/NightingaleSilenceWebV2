...
6. `NSGlamour`、`Plate` 等工具页不默认继承强像素装饰。
...
- 窗口标题栏保留 `pixel-soft` 原有渐变和轻重，只把装饰点替换为千禧年互联网小窗体感的左侧黑色方块图标、右侧最小化/最大化/关闭方形按钮。
...
- 文件：`src/styles/experiments/fantasy-card.css`
- 调性：西幻角色档案卡、冷色暮光背景、淡玫瑰纸面、酒红/墨绿/古铜点缀。
- 用途：观察和 `pixel-soft` 差异较大的角色展示方向，尤其适合 FF14 OC、创作角色档案、装备印象和故事片段。
- 限制：不是 DnD 规则角色卡，不默认引入 STR/DEX/技能检定等跑团数值体系；当前只展示 FF14 OC 氛围信息，例如职业、定位、故乡、装备印象、角色关键词和短故事。
- 资产：当前不引入第三方图片、字体或 UI 库；头像区域是 CSS 预览占位，后续如果正式使用，应替换为用户自己的角色图或站点自有素材。
- 边界：只作用于 `[data-style-preview='fantasy-card']`，不进入正式全站样式，也不影响 `pixel-soft`。
