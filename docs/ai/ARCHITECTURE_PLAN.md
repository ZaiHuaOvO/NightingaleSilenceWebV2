...
4. `设置 | CONFIG` 与 `菜单 | MENU` 是顶栏同级入口；`菜单` 管站点导航，`设置` 管全局显示模式等站点设置。
...
- `siteLabels`：站点稳定展示名和常用导航名，例如 `狒狒14工房`、`关于 | ABOUT`。
...
4. 像 `狒狒14工房` 这种会在首页、顶栏、返回链接、标题和可访问性标签中重复出现的稳定名词，必须从 `siteLabels`、`siteCategories` 或工具配置读取，不能散落写死。
