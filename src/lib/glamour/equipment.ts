...
export const GLAMOUR_SLOT_DEFINITIONS: GlamourSlotDefinition[] = [
  { key: 'MainHand', names: { zh: '主手', en: 'Main Hand', ja: 'メインアーム', ko: '주 무기', tc: '主手', fr: 'Main directrice', de: 'Haupthand' } },
  { key: 'OffHand', names: { zh: '副手', en: 'Off Hand', ja: 'サブアーム', ko: '보조 무기', tc: '副手', fr: 'Main non directrice', de: 'Nebenhand' } },
  { key: 'HeadGear', names: { zh: '头部', en: 'Head', ja: '頭', ko: '머리', tc: '頭部', fr: 'Tête', de: 'Kopf' } },
  { key: 'Body', names: { zh: '身体', en: 'Body', ja: '胴', ko: '몸통', tc: '身體', fr: 'Torse', de: 'Rumpf' } },
  { key: 'Hands', names: { zh: '手臂', en: 'Hands', ja: '手', ko: '손', tc: '手臂', fr: 'Mains', de: 'Hände' } },
  { key: 'Legs', names: { zh: '腿部', en: 'Legs', ja: '脚', ko: '다리', tc: '腿部', fr: 'Jambes', de: 'Beine' } },
  { key: 'Feet', names: { zh: '脚部', en: 'Feet', ja: '足', ko: '발', tc: '腳部', fr: 'Pieds', de: 'Füße' } },
  { key: 'Ears', names: { zh: '耳部', en: 'Ears', ja: '耳', ko: '귀', tc: '耳部', fr: 'Oreilles', de: 'Ohren' } },
  { key: 'Neck', names: { zh: '颈部', en: 'Neck', ja: '首', ko: '목', tc: '頸部', fr: 'Cou', de: 'Hals' } },
  { key: 'Wrists', names: { zh: '腕部', en: 'Wrists', ja: '腕', ko: '손목', tc: '腕部', fr: 'Poignets', de: 'Handgelenke' } },
  { key: 'LeftRing', names: { zh: '左指', en: 'Left Ring', ja: '左指', ko: '왼쪽 손가락', tc: '左指', fr: 'Bague gauche', de: 'Finger (links)' } },
  { key: 'RightRing', names: { zh: '右指', en: 'Right Ring', ja: '右指', ko: '오른쪽 손가락', tc: '右指', fr: 'Bague droite', de: 'Finger (rechts)' } },
  { key: 'Glasses', names: { zh: '面部配饰', en: 'Facewear', ja: 'フェイスアクセサリー', ko: '얼굴 소품', tc: '臉部配件', fr: 'Accessoires de visage', de: 'Gesichtsaccessoires' } },
  { key: 'FashionAccessory', names: { zh: '时尚配饰', en: 'Fashion Accessory', ja: 'FASHION ACCESSORIES', ko: '패션 소품', tc: '時尚配件', fr: 'Accessoires de mode', de: 'MODEACCESSOIRES' } }
]
...
export function createDyeEntryFromStain(stain: GlamourStain, locale: GlamourLocale): GlamourDyeEntry {
...
export function getEquipmentDyeSummary(
  ...
  if (dyeCount <= 0) {
    const text =
      resolveLocalized(candidate.dye_display_by_locale, locale) || cleanGlamourText(candidate.dye_display)
  ...
  if (!dyeText || getCopyDyeEntries(getDisplayDyeEntries(candidate, entry.slot, noDyeLabels, locale)).length === 0) {
...
export function getEquipmentModelCode(entry: GlamourEquipmentEntry): string {
  ...
  if (Number.isFinite(Number(model.set))) {
    return [model.set, model.base, model.variant, 0]
...
function normalizeEquipmentEntry(value: unknown): GlamourEquipmentEntry | undefined {
  ...
  const candidates = Array.isArray(value.candidates)
    ? value.candidates.map(normalizeCandidate).filter((candidate): candidate is GlamourCandidate => Boolean(candidate))
    : []
...
export function getVisibleEquipmentEntries(
  ...
  return GLAMOUR_SLOT_DEFINITIONS.map(
    (slot) => entriesBySlot.get(slot.key) ?? makeEmptyEquipmentEntry(slot.key, payload)
  )
}
